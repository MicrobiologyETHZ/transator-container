...
RUN apk update && apk add openjdk8 maven wget && \
    ...
    mvn clean install -X -f /transator-java/pom.xml -DskipTests && \
    mvn clean package -X -f /transator-java/pom.xml -DskipTests
...
COPY --from=transator /transator-java/PKSPredictorWeb/target/PKSPredictorWeb.war $CATALINA_HOME/webapps/trans-AT-PKS.war
COPY --from=transator /transator-java/PKSPredictorREST/target/PKSPredictorREST.war $CATALINA_HOME/webapps/trans-AT-PKS#rest.war
COPY --from=transator /transator-java/Common/target/Common-2.0.jar /java-runner/Common-2.0.jar
COPY --from=transator /transator-java/PKSPredictorRunner/target/PKSPredictorRunner-2.0.jar /java-runner/PKSPredictorRunner.jar
...
# ENV CATALINA_OPTS=-verbose:class
